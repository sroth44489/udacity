...
class LearningAgent(Agent):
    ...
    def get_maxQ(self, state):
        ...
        if state in self.Q:
            ...
            for action in q_action_entries:
                if maxQ is None:
        ...
        if self.learning:
            if state in self.Q:
                ...
                for action in self.valid_actions:
                    q_action_entry[action] = 0
                self.Q[state] = q_action_entry
    ...
    def choose_action(self, state):
        ...
        if not self.learning:
            ...
            if random.random() < self.epsilon:
                ...
                if state in self.Q:
                    q_action_entries = self.Q[state]
                    for test_action in q_action_entries:
                        if maxQ is None:
    ...
    def learn(self, state, action, reward):
        ...
        # For this algorithm, we are not using the gamma term, so it reduces to
        # Q(s,a) := Q(s,a) + alpha * ( r - Q(s,a) )
        # or
        # Q(s,a) := r*alpha + (1-alpha)*Q(s,a)

        self.Q[state][action] = reward * self.alpha + (1.0 - self.alpha) * self.Q[state][action]
        return
